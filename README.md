...
## OCR (распознавание текста, раздел 13 ТЗ)

Работает полностью локально в браузере через официальный
[`PaddleOCR.js`](https://www.npmjs.com/package/@paddleocr/paddleocr-js)
(WASM/ONNX Runtime Web) — без отдельного backend-сервиса и без внешнего
платного API. Модель распознаёт кириллицу (`lang: 'ru'`).

Важно после `npm install` у себя (в песочнице Claude сети не было, см.
`PROGRESS.md` → «Часть 5» для подробностей):

1. Проверить `npm run dev`/`npm run build` — сама интеграция написана по
   документации пакета, но версия/форма ответа `predict()` не была
   сверена вживую. Если что-то не совпадёт — правки нужны только в
   `src/lib/ocr/paddleOcr.ts` (функция `normalizeItem`), остальной код
   OCR-модуля от точной формы ответа не зависит.
2. Первое распознавание в браузере скачает ONNX-модель и WASM-рантайм с
   CDN (jsDelivr / CDN самого пакета) — это разовая загрузка, дальше
   браузер кеширует. Если нужен полностью офлайн-вариант без единого
   внешнего запроса в рантайме — скачать веса из
   `node_modules/@paddleocr/paddleocr-js` вручную, положить в
   `public/` и передать локальные пути в `PaddleOCR.create()`
   (см. README пакета — состав параметров может отличаться от версии,
   описанной в `paddleOcr.ts`).
3. `netlify.toml` включает заголовки `Cross-Origin-Opener-Policy:
   same-origin` и `Cross-Origin-Embedder-Policy: credentialless` —
   нужны только для многопоточного WASM (ускоряет OCR), без них движок
   продолжит работать, просто медленнее. После деплоя стоит один раз
   вручную проверить, что превью изображений в `/documents/:id`
   по-прежнему открываются (см. комментарий в `netlify.toml`, почему
   выбран именно `credentialless`, а не `require-corp`).
